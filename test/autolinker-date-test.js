...
describe('autolinker-date', function(){
	...
	it('a set date in "9 May 2014 at 16:10" format', function(done){
		...
		autolinker.parse(options, function(err, result){
			assert.equal(result.html, 'An appointment on <time class=\"auto-date-discovery\" datetime=\"2014-05-09T16:10\">9 May 2014 at 16:10</time>', 'should convert to html');
			assert.equal(err, null, 'should excute without error');
			done();
		});
	});
	...
	it('a set date in "10 May 2014 at 16:10" format', function(done){
		...
		autolinker.parse(options, function(err, result){
			assert.equal(result.html, 'An appointment on <time class=\"auto-date-discovery\" datetime=\"2014-05-10T16:10+02:00\">10 May 2014 at 16:10</time>', 'should convert to html');
			assert.equal(err, null, 'should excute without error');
			done();
		});
	});
	...
	it('a set date in "13 May 2014 at 16:10 EDT" format', function(done){
		...
		autolinker.parse(options, function(err, result){
			assert.equal(result.html, 'An appointment on <time class=\"auto-date-discovery\" datetime=\"2014-05-13T16:10-04:00\">13 May 2014 at 16:10 EDT</time>', 'should convert to html');
			assert.equal(err, null, 'should excute without error');
			done();
		});
	});
